...
/// - On macOS, the limit is 255 characters traditionally, but after personal testing it seems to be
///   around 298 on my (@arlyon) machine.
...
fn validate_path_length<'a>(path: &'a Path) -> Result<Cow<'a, Path>> {
    /// Here we check if the path is too long for windows, and if so, attempt to canonicalize it
    /// to a UNC path.
    #[cfg(target_family = "windows")]
    fn validate_path_length_inner<'a>(path: &'a Path) -> Result<Cow<'a, Path>> {
    ...
    fn validate_path_length_inner<'a>(path: &'a Path) -> Result<Cow<'a, Path>> {
        // on most linux distros this is likely a lot larger than 1016, but macOS is *special*
        const MAX_PATH_LENGTH: usize = 1016;
        ...
        if path.as_os_str().len() > MAX_PATH_LENGTH {
            return Err(anyhow!(
                "path is too long (exceeds {} bytes)",
                MAX_PATH_LENGTH
            ));
        }
